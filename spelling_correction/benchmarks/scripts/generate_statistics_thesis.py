...
def generate_statistics(args: argparse.Namespace) -> None:
    ...
    for group, splits in benchmark_groups.items():
        ...
        for split in splits:
            ...
            for corrupt_line, correct_line, correct_sequence in zip(corrupt_lines, correct_lines, correct_sequences):
                ...
                if args.benchmark_type == "sec":
                    for corrupt_word, correct_word in zip(corrupt_words, correct_split):
                    ...
                    for corrupt_word, is_error in zip(corrupt_words, correct_split):
                        if int(is_error):
                            if corrupt_word in dictionary:
                                real_word_errors += 1
                            else:
                    ...
                    for corrupt_word, correct_word in zip(corrupt_words, correct_sequence.split()):
                        if corrupt_word != correct_word:
                            if corrupt_word in dictionary:
                                real_word_errors += 1
                            else:
    ...
    header = [
        "Benchmark", "Number of sequences", "Number of words",
        "Total word errors", "Word error rate", "Real word errors", "Non-word errors"
    ]
    is_sed_sequence = args.benchmark_type == "sed_sequence"
    if is_sed_sequence:
        header.extend(["Sequences with errors", "Sequence error rate"])
    data = []
    horizontal_lines = []
    for group in groups:
        ...
        for split in splits:
            num_seq, num_words, rw_err, nw_err, total_err, additional = statistics[group][split]
            ...
            data.append([
                f"{group}:{split}", f"{num_seq:,}", f"{num_words:,}",
    ...
    save_text_file(args.out_file, [table])
