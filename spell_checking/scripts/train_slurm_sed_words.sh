...
if [[ $approach == "gnn" ]]; then
  ...
  for approach_idx in ${!approaches[@]}; do
    ...
    config="$config_dir/train/sed_words.yaml"
    rel_config=$(realpath "$config" --relative-to "$workspace")
    export NSC_CONFIG=$rel_config
    echo "Starting approach $approach with config $(realpath $config --relative-to $config_dir) and parameters:"
  ...
  for approach_idx in ${!approaches[@]}; do
    ...
    config="$config_dir/train/sed_words_transformer.yaml"
    rel_config=$(realpath "$config" --relative-to "$workspace")
    export NSC_CONFIG=$rel_config
    echo "Starting approach $approach with config $(realpath $config --relative-to $config_dir) and parameters:"
